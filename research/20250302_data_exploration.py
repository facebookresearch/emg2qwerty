...
# %%
def explore_hdf5(
    item: Union[h5py.Group, h5py.Dataset, h5py.File], indent: int = 0
) -> None:
    ...
    if isinstance(item, h5py.Group):
        ...
        print(
            " " * indent
            + f"DATASET: {item.name}, Shape: {shape_str}, Type: {dtype_str}"
        )
...
def plot_emg_data(emg_data, n_samples=500):
    ...
    fig, axes = plt.subplots(
        total_channels, 1, figsize=(15, 2 * total_channels), sharex=True
    )
    ...
    for i in range(n_channels_right):
        right_channel_data.append(
            np.array([entry["emg_right"][i] for entry in emg_data[:n_samples]])
        )

    # Collect left hand channel data
    for i in range(n_channels_left):
        left_channel_data.append(
            np.array([entry["emg_left"][i] for entry in emg_data[:n_samples]])
        )

    # Find min and max for right hand channels
    right_min = (
        min(np.min(data) for data in right_channel_data) if right_channel_data else 0
    )
    right_max = (
        max(np.max(data) for data in right_channel_data) if right_channel_data else 1
    )

    # Find min and max for left hand channels
    left_min = (
        min(np.min(data) for data in left_channel_data) if left_channel_data else 0
    )
    left_max = (
        max(np.max(data) for data in left_channel_data) if left_channel_data else 1
    )
...
if emg_data is not None:
    ...
    plt.subplot(2, 1, 1)
    plt.plot(emg_data)
    plt.title("Full EMG Timeseries: /emg2qwerty/timeseries")
...
def analyze_channel_correlations(
    emg_data, side="both", method="pearson", threshold=0.8, plot=True
):
    """
    ...
    if side in ["right", "both"]:
        ...
        right_data = np.array(
            [
                [entry["emg_right"][i] for i in range(n_channels_right)]
        ...
        right_df = pd.DataFrame(
            right_data, columns=[f"Right_Ch{i + 1}" for i in range(n_channels_right)]
        )
    ...
    if side in ["left", "both"]:
        ...
        left_data = np.array(
            [
                [entry["emg_left"][i] for i in range(n_channels_left)]
                for entry in emg_data
            ]
        )
...
def reduce_emg_channels(
    emg_data, correlation_threshold=0.8, method="pearson", reduction_method="pca"
):
    """
    ...
    print(
        f"Right hand: {n_channels_right} → {len(reduction_info['right']['channel_groups'])} channels"
    )
...
def plot_reduced_emg_data(original_data, reduced_data, reduction_info, n_samples=500):
    ...
    fig = plt.figure(
        figsize=(18, max(n_orig_right + n_orig_left, n_reduced_right + n_reduced_left))
    )
    ...
    for i in range(n_orig_right):
        ...
        channel_data = np.array(
            [entry["emg_right"][i] for entry in original_data[:n_samples]]
        )
    ...
    for i in range(n_reduced_right):
        ...
        channel_data = np.array(
            [entry["emg_right"][i] for entry in reduced_data[:n_samples]]
        )
        ax.plot(time_data, channel_data)
    ...
    for i in range(n_orig_left):
        ...
        channel_data = np.array(
            [entry["emg_left"][i] for entry in original_data[:n_samples]]
        )
        ax.plot(time_data, channel_data)
    ...
    for i in range(n_reduced_left):
        ax = plt.subplot2grid(
            (n_reduced_right + n_reduced_left, 2), (i + n_reduced_right, 1)
        )
        channel_data = np.array(
            [entry["emg_left"][i] for entry in reduced_data[:n_samples]]
        )
        ax.plot(time_data, channel_data)
    ...
    plt.tight_layout()
    plt.suptitle(
        f"Comparison of Original vs. Dimensionality-Reduced EMG Data\nReduction Method: {reduction_info['right']['method']}",
        fontsize=16,
        y=1.02,
    )
    plt.subplots_adjust(top=0.95)  # Make room for the title
...
# %%
reduced_emg_data, reduction_info = reduce_emg_channels(
    emg_data, correlation_threshold=0.8, method="pearson", reduction_method="pca"
)
# %%
...
file = h5py.File(
    "data/89335547/2021-06-02-1622679967-keystrokes-dca-study@1-0efbe614-9ae6-4131-9192-4398359b4f5f.hdf5",
    "r",
)
emg2qwerty_group: h5py.Group = file["emg2qwerty"]
timeseries: h5py.Dataset = emg2qwerty_group["timeseries"]
